...
if [[ -z "${OAUTH_ISSUER}" || -z "${OAUTH_AUDIENCE}" || -z "${REPOSITORY_URL}" ]]; then
  echo "Set AWR_OAUTH_ISSUER, AWR_OAUTH_AUDIENCE, and AWR_REPOSITORY_URL before deploying." >&2
  exit 1
fi
...
ENV_VARS="AWR_ENV=production,AWR_AUTH_MODE=oauth,AWR_STORAGE=firestore,AWR_EXECUTOR=cursor_cloud,AWR_REPOSITORY_URL=${REPOSITORY_URL},AWR_BASE_REF=${BASE_REF},AWR_OAUTH_ISSUER=${OAUTH_ISSUER},AWR_OAUTH_AUDIENCE=${OAUTH_AUDIENCE},GOOGLE_CLOUD_PROJECT=${PROJECT_ID},FIRESTORE_DATABASE=${FIRESTORE_DATABASE}"
