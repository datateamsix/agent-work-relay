...
def build_service(
    ...
    return BrokerService(
        ...
        default_repository_url=os.getenv("EWB_REPOSITORY_URL") or None,
        default_base_ref=os.getenv("EWB_BASE_REF", "main"),
    )


def _build_executor() -> PlanningExecutor:
    selected = os.getenv("EWB_EXECUTOR", "recording_cursor")
    if selected == "recording_cursor":
        return RecordingCursorExecutor()
    if selected == "cursor_cloud":
        api_key = os.getenv("CURSOR_API_KEY", "")
        if not api_key:
            raise RuntimeError("CURSOR_API_KEY is required when EWB_EXECUTOR=cursor_cloud.")
